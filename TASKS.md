...
- [x] P2.0 激活统计采集
- [x] P2.1 逐通道缩放校准算法
- [x] P2.2 跨分布鲁棒性实验（本阶段只测 AWQ 自身跨分布矩阵，不含 GPTQ——GPTQ 对比是 P2.4 云端阶段的范围，见 project_plan_v9.md §8『量化自研对象』『量化对比基线工具』两行）
- [x] P2.3 校准集大小消融
- [ ] P2.4（Mac部分）`mlx_lm.awq` 交叉验证（可提前，不依赖 P1；未在本次范围内完成）
