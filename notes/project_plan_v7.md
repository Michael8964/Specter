# Specter 项目计划 v7 —— 读完参考文档全文（28页）后的补全版

v6 只读了参考文档（`TWO_WAY_BREAKTHROUGH_zh.pdf`）的前18页就动手写了。读完剩余10页（第19-28页：完整的端到端叙事走读、附录A只读诊断查询、附录B分类文献来源、附录C含公式/prompt/worked example的实现细节）之后发现，v6 补的五块东西（指标体系/机制拆分/走读/叙事/附录）**结构上都有了，但颗粒度都比参考文档浅一层**。具体四个缺口：

1. **走读只有happy path**。参考文档的Priya故事里有一次交换"胎死腹中"（12小时沉默、擦肩而过的时刻、最后靠系统的周期性提醒才救回来）——故事本身在演示系统怎么处理失败/边缘情况，不只是演示成功路径。v6 的走读五步全是顺利执行。
2. **公式没有代入真实数字算一遍**。参考文档的R1匹配分公式后面跟着一个完整的数值例子（A→B、B→A的具体值，算出R*=0.576，还对照了单向算术平均会算出0.299来说明"不对称在构造上就受到惩罚"）。v6 的附录A只有公式和伪代码，没有代入数字验证过一遍。
3. **文献引用是"详见xxx.md"的指针，不是真的写在文档里**。参考文档附录B是一份三十多条、按主题分类、每条都写清楚发现了什么百分比/结论的真实文献列表，直接摆在文档里。v6 的第6节只说"详见prior_art.md"。
4. **附录里的"只读查询"是可以直接执行的代码（真实SQL），不是字段列表**。v6 附录C只列了要记录哪些字段，没有列"记录完之后怎么读出结论"的分析代码。

v7 把这四处全部补上，不引入新的架构/技术决策——v6 的所有设计决策原样保留。另外借这次通读，把参考文档里v6没抄的第五个模式也补了：一段"研究诚信护栏"（参考文档有一份"风险台账"，防止用户/平台被系统性地欺骗；Specter没有用户，但有类比风险——防止自己在评测/调参过程中无意识地欺骗自己），写成新的9.6节。
...
| **v7**（本版本） | 读完参考文档全部28页后补四处颗粒度缺口：走读加入失败/恢复场景、公式配数值算例、真实文献列表内嵌（而非外链）、附录C加可执行的诊断分析代码；新增"研究诚信护栏"一节 |
...
6. **测量本身也要接受怀疑**(v7新增,呼应9.6)——一个"符合预期"的实验结果不会自动免检;必须先证明测量工具本身是可靠的(测试的测试),再采信结果。
...
条件性结论(完整分类文献列表见**附录E**,不再用外链指针代替内容——这是v7相对v6的修正之一): 投机解码/量化已是主流框架标配, 真正区分度在手写实现+正确性验证+边界刻画; IBM/PyTorch生产复盘证实代码/结构化输出场景效果最好, batch>64开始吞吐下降; AWQ相对GPTQ对校准集分布更鲁棒; 2024-2026自适应投机控制是活跃前沿方向, 且动态γ调整已是HF Transformers默认行为(4.45.0起)。
...
### 9.6 研究诚信：防止自己欺骗自己（v7新增）

> 参考文档有一份"风险台账与反刷设计"，防止平台被用户系统性钻空子。Specter 没有外部用户，类比风险不是"被别人刷"，而是**评测/调参过程中无意识地让结果看起来比实际更好**——这在没有同行评审、只有自己既是研究者又是评审者的个人项目里尤其容易发生，必须显式设计对策，而不是假设自己足够客观。
...
| 6 | P2.4+P4.2+P5.2+P5.4 云端规模化验证 | M(3天) | 云端,$30-50 | LLM Compressor对比数字;vLLM对比数字;量化-γ偏移结论;HF基线对比数字;held-out集跑一次最终确认 |
...
## 13. 端到端走读（计划中的产出格式，非已实测数字；v7扩写：加入失败与恢复场景）

> 对标参考文档 §8"一天的旅程"。参考文档的故事里有一次交换差点"胎死腹中"——用叙事本身去检验系统在不顺利情况下是否还立得住,不只是演示顺利路径。v7 把这个结构补进来：下面的走读不再只有一条顺利执行的任务,还包含一次"控制器一度做出错误决策、系统靠自己的补救机制而不是运气修正过来"的场景。
...
- 简历bullet(见附录D)
- 本系列计划文档(v1-v7)作为项目思考过程的书面记录，可选择性放入repo的`docs/`目录
...
v7(本版本)：读完参考文档全部28页后补齐颗粒度缺口，内容决策不变，等待用户确认后再讨论是否进入阶段0。
...
可以看到：只要连续命中"全部接受"，γ 会持续扩窗式增长（3→5→7→...直到γ_max）；一旦某一轮没有全部接受，立刻回落到EMA的保守估计，不会让γ在一次不理想的输出后继续野蛛式增长——这是"扩窗要快，收缩要稳"的设计意图在数字上的体现，也是P5.1波动场景测试要验证的核心行为。
...
> 对标参考文档里引用自己产品日志(`tx_demand_search_log`等)和附录A"只读查询"的做法——v6只列了要记录哪些字段,v7把"记下来之后怎么读出结论"的分析代码也写出来，实现阶段可以直接照着跑。Specter没有生产数据库，日志落地为本地文件（jsonl/parquet），下面的"查询"是pandas风格的伪代码,不是真实SQL。
...
# 量化程度对最优γ的偏移（对应P5.2，坑10核心实验）
optimal_gamma_by_quant = (
    df_steps.groupby("quantization_level")
    .apply(lambda g: g.loc[g["latency_ms"].idxmin(), "gamma_used"])
)
shift_pct = (
    (optimal_gamma_by_quant.max() - optimal_gamma_by_quant.min())
    / optimal_gamma_by_quant.min() * 100
)  # 对标SpecKV的41.2%
...
## 附录E：分类文献与信息来源（v7新增——内嵌全文，不再用外链指针代替内容）
...
### 五、自适应投机步长控制（2024-2026前沿）
- Kim et al. 2025 (arXiv 2504.00030, Texas A&M)，*Token-Driven GammaTune* —— P5.0核心算法来源，Algorithm 1（EMA更新+扩窗）直接实现，SpecBench上平均15%±5%提速，Limitations章节明确承认低方差场景收益有限、对抗性场景会退化（坑9）。
- SpecKV 2026 (arXiv 2605.02888)，*Adaptive Speculative Decoding with Compression-Aware Gamma Selection* —— 最优γ随量化程度偏移最高41.2%的数字来源（坑10，P5.2实验对标对象），其HF Transformers手写实现方法论是Specter Mac阶段框架选择的直接先例。
- Agrawal et al. 2024 (arXiv 2410.18351)，*AdaEDL: Early Draft Stopping via Entropy-based Lower Bound* —— 熵基提前停止判据（附录A原有的$1-\sqrt{\gamma H_{DM}(x)}<\lambda$公式来源），DL=3/7/16下接受token数标准差递增(1.2/1.92/2.35)的数据来源，只作文献对比不重新实现（§3非目标3）。
- BanditSpec 2025 (arXiv 2505.15141)，*Adaptive Speculative Decoding via Bandit Algorithms* —— UCB/EXP3老虎机框架，regret bound理论，实现成本过高只作文献对比（坑12），其"忽视batch size/KV cache切换开销"的设计缺陷是P5.3必须实测切换开销的直接依据（坑11）。
- Nightjar 2025 (arXiv 2512.22420)，*Dynamic Adaptive Speculative Decoding for LLM Serving* —— 高负载30.25%吞吐倒退的数字来源（坑7，P4.2对标对象），批评DSD"重激活难题"和BanditSpec"忽视切换开销"，直接决定P5.3熔断器必须含周期性重探测机制的设计（坑11）。
- UC Berkeley硕士论文 *TurboSpec* (EECS-2025-224，未下载全文，校内链接失效)——offline profiling+online feedback动态调参、提出"goodput"统一指标，方向上最接近生产级闭环控制系统，仅供参考，不作为可引用数据来源。
- 背景事实：动态γ调整已是Hugging Face Transformers默认行为（4.45.0起，Mamou et al. 2024方法）——P5.4的生产基线对比对象。
